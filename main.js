...
function studyRewardBoost(studyNum,rewardNum) {
	if (rewardNum===2) return studies[10].reward(4)
	if (rewardNum===3) {
		let out = lightCache.currentEffect[0]
		let studyAchievements = [null,608,609,705,715,812,814]
...
const masteryData = {
	...
	111:{icon:"<span class=\"_mastery\">M<sub>104</sub></span>→<span class=\"_prismatic\">P</span>"},
	112:{icon:"<span class=\"_mastery\">M</span><span class=\"xscript\"><sup>+</sup><sub class=\"_mastery\">104</sub></span>"}
}
...
function masteryEffect(x) {
	...
	if (x===111) return c.d0_05.mul(masteryBoost(111))
	if (x===112) return g.masteryPower.add(c.e10).log10().log10().pow(masteryBoost(112).div(c.d10))
...
function masteryFormula(x) {
	...
	if (x===111) return masteryBoost(111).mul(c.d0_05).noLeadFormat(3)
	if (x===112) return "log<sup>[2]</sup>(MP + "+c.e10.format()+")<sup>"+masteryBoost(112).div(c.d10).noLeadFormat(3)+"</sup>"
...
function masteryBaseText(x) {
	...
	if (x===101) return "The \"Wormhole to Somewhere\" achievement reward is raised to the power of {}"+(achievement(501).effectExp(false).eq(c.d1)?"":("<br><span class=\"small\">(if inactive: "+achievement(501).effectExp(false).format(3)+")</span>"));
	...
	if (x===111) return "Mastery 104 affects prismatic gain with ^{} effect"
...
const autobuyers = {
	...
	research:{baseInterval:60,baseCost:N("5.55e55555"),costGrowth:c.d1_05,resource:"knowledge",unlockReq:function(){return g.achievement[817]}},
};
const autobuyerMeta = {
...
function starCost(x=g.stars,gal=g.galaxies) {
	...
	if (g.achievement[811]) cost = cost = [cost,N(0.97),N(g.highestGalaxiesSpacetime-gal)].decimalPowerTower()
...
function darkStarReqFormula() {
	...
	out = "(("+out+(g.darkstars.gte(start)?"<br>":"")+" + 22)<sup>2</sup> - 196)"+formulaFormat.exp(darkStarPriceMod("pow"))+formulaFormat.mult(N(0.125).div(darkStarPriceMod("div")))+formulaFormat.add(darkStarPriceMod("sub").neg())
...
function StudyE(x) {
	if (x<10) if (StudyE(10)) if ([[1,4,7],[2,5,8],[3,6,9],g.study10Options].includes(x)) return true
...
function updateAllStudyDivs() {for (let i=1;i<Object.keys(studies).length;i++) {updateStudyDiv(i);}}
function currentStudyResearch() {return (g.activeStudy===0)?"":(g.activeStudy===10)?studies[10].research[studyPower(10)]:studies[g.activeStudy].research}
	...
	if ((x===10)&&(studyPower(10)===3)&&(g.study10Options.length<3)) { // pick options
		...
		if (StudyE(5)) {
			...
			if (g.activeStudy!==0) { // study 10 proofing
				let studyRes = currentStudyResearch()
...
function updateLightCache(i){
	...
	if (i===5) {
		...
		lightCache.nextEffect[i] = lightEffect[i].value(g.lumens[i].add(c.d1))
	}
...
function antiAxisDimBoost(type,next=false) {
	...
	return x.div(c.e3).add(c.d1).ln().add(c.d1).pow(c.d0_9).sub(c.d1).mul(antiAxisDimBoostPower(type)).add(c.d1)
}
function antiAxisDimBoostFormula(type){return "(ln("+type+" ÷ 1,000 + 1)<sup>0.9</sup> - 1) × "+antiAxisDimBoostPower(type).mul(c.e2).noLeadFormat(3)+"%"}

const topResources = [
	{
		text:function(){return "<span class=\"_exoticmatter\">"+g.exoticmatter.format()+"</span> exotic matter"+((g.exoticmatter.lt(c.ee9)||g.exoticmatter.gt_tolerance(stat.exoticmatterPerSec,0.01))?(" (<span class=\"_exoticmatter\">"+stat.exoticmatterPerSec.noLeadFormat(2)+"</span> / s)"):"");},
		condition:function(){return g.topResourcesShown.exoticmatter;}
	},
	{
		text:function(){return "<span class=\"_mastery\">"+g.masteryPower.format()+"</span> mastery power"+((g.masteryPower.lt(c.ee9)||g.masteryPower.gt_tolerance(stat.masteryPowerPerSec,0.01))?(" (<span class=\"_mastery\">"+stat.masteryPowerPerSec.format(2)+"</span> / s)"):"");},
	...
	{
		text:function(){return g.stardustUpgrades[4]>0?("<span class=\"_darkmatter\">"+g.darkmatter.format()+"</span> dark matter"+((g.darkmatter.lt(c.ee9)||g.darkmatter.gt_tolerance(stat.darkmatterPerSec,0.01))?(" (<span class=\"_darkmatter\">"+stat.darkmatterPerSec.format(2)+"</span> / s)"):"")):"";},
	...
	{
		text:function(){return g.studyCompletions[9]>0?("<span class=\"_antimatter\">"+g.antimatter.format()+"</span> antimatter"+((g.antimatter.lt(c.ee9)||g.antimatter.gt_tolerance(stat.antimatterPerSec,0.01))?(" (<span class=\"_antimatter\">"+stat.antimatterPerSec.format(2)+"</span> / s)"):"")):"";},
