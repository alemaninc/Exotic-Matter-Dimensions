	...
	generate: function(layer) {
		...
		switch (layer) {
			case 1:
				...
				out.gain = stat.pendingstardust;
				break;
			case 2:
				out.time = g.timeThisWormholeReset;
				out.truetime = g.truetimeThisWormholeReset;
				out.gain = stat.pendinghr;
				out.efficiency = stat.pendinghr.div(g.timeThisWormholeReset);
				out.study = g.activeStudy
				break;
			default:
				throw "Cannot access previousPrestige.generate("+layer+")";
		}
		return out;
	},
	stardustRunsStored:[
  ...
  showBuild:function(layer,type,index){
    ...
    if (layer=="stardust") {
      if (type=="last") {
        ...
        throw "Cannot access previousPrestige.showBuild("+layer+","+type+","+index+")"
      }
    ...
    popup({
      text:[
        ["Stars",g.ownedStars.join(","),true],
        ["Masteries",this.stars().join(","),true],
        ["Research",g.ownedResearch.join(","),unlocked("Hawking Radiation")]
