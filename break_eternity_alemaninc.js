...
(function (global, factory) {
  ...
  function () {
    ...
    Decimal.linearSoftcap = function (value,start,power,layer=0) {
      ...
      return start.mul(N(1).add(power.add(1).mul(value.div(start).sub(1))).root(power.add(1))).layerf("{x}+"+layer)
    }
    ...
    Decimal.semilogSoftcap = function (value,start,power,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      power=new Decimal(power)
      return Decimal.pow(start,value.mul(power.add(1)).sub(start).log(start).root(power.add(1))).layerf("{x}+"+layer)
    }
    ...
    Decimal.logarithmicSoftcap = function (value,start,power,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      power=new Decimal(power)
      return value.div(start).ln().mul(power).add(1).root(power).mul(start).layerf("{x}+"+layer)
    }
    ...
    Decimal.superlogSoftcap = function (value,start,power,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      power=new Decimal(power)
      return value.div(start).pow(power).quad_slog(c.e).add(1).root(power).mul(start).layerf("{x}+"+layer)
    }
    ...
    Decimal.convergentSoftcap = function (value,start,limit,layer=0) {
      ...
      value=new Decimal(value).layerf("{x}-"+layer)
      start=new Decimal(start).layerf("{x}-"+layer)
      limit=new Decimal(limit).layerf("{x}-"+layer)
      return limit.sub(limit.sub(start).div(N(1).add(value.sub(start).div(limit.sub(start))))).layerf("{x}+"+layer)
    }
    ...
    Decimal.linearScaling = function (value,start,power,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      power=new Decimal(power)
      return start.div(power.add(1)).mul(power.add((value.div(start)).pow(power.add(1)))).layerf("{x}+"+layer)
    }
    ...
    Decimal.semiexpScaling = function (value,start,power,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      power=new Decimal(power)
      return start.pow(value.log(start).pow(power.add(1))).div(power.add(1)).add(start.mul(N(1).sub(power.add(1).pow(-1)))).layerf("{x}+"+layer)
    }
    ...
    Decimal.exponentialScaling = function (value,start,power,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      power=new Decimal(power)
      return value.div(start).pow(power).sub(1).div(power).exp().mul(start).layerf("{x}+"+layer)
    }
    ...
    Decimal.superexpScaling = function (value,start,power,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      power=new Decimal(power)
      return c.e.quad_tetr(value.div(start).pow(power).sub(1)).root(power).mul(start).layerf("{x}+"+layer)
    }
    ...
    Decimal.divergentScaling = function (value,start,limit,layer=0) {
      ...
      start=new Decimal(start).layerf("{x}-"+layer)
      limit=new Decimal(limit).layerf("{x}-"+layer)
      let output=limit.sub(start).mul(limit.sub(start).div(limit.sub(value)).sub(1)).add(start).layerf("{x}+"+layer)
...
*/
    ...
    Decimal.prototype.layerf = function (code) {      // This is a function which changes the layer of a number according to code which is input. For example, layerf("{x}**2") will square the layer. Therefore, N(1e10).layerf("{x}*1.5") returns 10^^3, because slog(1e10)=2 and 2*1.5=3.
      ...
      let height = eval(code.replace(/{x}/g,x))
    ...
    Decimal.prototype.dilate = function (exponent,layer) {
      ...
      return safeguard.layerf("{x}-"+layer).safepow(exponent).layerf("{x}+"+layer)
    }
...
function gformat() {   // value, precision (significant figures for small numbers), notation
  ...
  if (x.sign==-1) return "-"+gformat(x.abs(),arguments[1],arguments[2])
  if (arguments[1]==undefined) arguments[1]=0
  if (arguments[2]==undefined) arguments[2]=g.notation
  if (x.eq(0)) return "0"
  if (x.lt(1e-5)) return "(1 / "+gformat(x.pow(-1),arguments[1],arguments[2])+")"
  ...
  if (arguments[2]=="Alemaninc Ordinal") {
  ...
  } else if (arguments[2]=="BE Default") {
    return x.toExponential(3)
  } else if (arguments[2]=="Engineering") {
    if (x.lt("eeeee6")) {
      let es = x.layer-((x.mag<1e6)?1:0)
      return Array(es+1).join("e")+format.engineering(x.layerf("{x}-"+String(es)),es==0?2:3)
    }
    ...
    return "E"+((height<1e12)?(gformat(x.layerf("{x}%1"),3,"")):"")+"#"+format.engineering(height,3)
  } else if (arguments[2]=="Hyper-E") {
    let height = x.quad_slog(10).floor().toNumber()
    if (height<1e6) return "E"+x.layerf("{x}%1").mul(1e3).floor().div(1e3).toString().substr(0,5)+"#"+height.toLocaleString("en-US")
    return "E"+((height<1e12)?(gformat(x.layerf("{x}%1"),3,"")):"")+"#"+format.scientific(height,3)
    ...
    if (x.lt("eeeee6")) {
      let es = x.layer-((x.mag<1e6)?1:0)
      return Array(es+1).join("e")+format.mixedscientific(x.layerf("{x}-"+String(es)),es==0?2:3)
    }
    let height = x.quad_slog(10).floor().toNumber()
    if (height<1e6) return "E"+x.layerf("{x}%1").mul(1e3).floor().div(1e3).toString().substr(0,5)+"#"+height.toLocaleString("en-US")
    return "E"+((height<1e12)?(gformat(x.layerf("{x}%1"),3,"")):"")+"#"+format.mixedscientific(height,3)
  } else if (arguments[2]=="Scientific") {
    if (x.gt("10^^6")) return gformat(x,0,"Hyper-E")
    let es = x.layer-((x.mag<1e6)?1:0)
    return Array(es+1).join("e")+format.scientific(x.layerf("{x}-"+String(es)),es==0?2:3)
