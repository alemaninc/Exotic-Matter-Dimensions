...
function EMfix(x) {
	...
	for (let i=0;i<vars.length;i++) g[vars[i]] = N(x)
}
function timeFix(x) {
...
function loadOldSave(savegame) {
	...
	if (axisCodes.map(x => savegame["dark"+x+"Axis"]).reduce((x,y)=>x+y)>=1000) {
		...
		g.darkTAxis=N(25)
		g.darkstars=N(25)
		g.ownedAchievements=[101,102,103,104,105,106,107,108,109,110,111,112,113,115,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,301,302,306,307,308,311,312,402,403,404,405,406,407]
		...
		g.darkVAxis=N(2)
		g.darkstars=N(4)
		g.ownedAchievements=[101,102,103,104,105,107,108,109,110,111,112,113,114,115,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,301,302,308,311,312,402,403]
