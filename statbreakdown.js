...
breakdownStats.exoticmatterPerSec={
	...
	modifiers:[
		...
		},
		{
			...
			mod:function(){return AchievementE(114)?achievement(114).effect().div(100).add(1):N(1);},
			func:function(prev){return prev.mul(this.mod());},
		...
		},
		{
			...
			mod:function(){return AchievementE(309)?g.masteryPower.add(1).pow(achievement(309).effect()):N(1);},
			func:function(prev){return prev.mul(this.mod());},
			text:function(){return "× "+this.mod().format(2)+SSBsmall(g.masteryPower.add(1).format(2),achievement(309).effect().format(4),3);}
		},
		{
			label:achievement.label(311),
			mod:function(){return AchievementE(311)?achievement(311).effect():N(1);},
			func:function(prev){return prev.mul(this.mod());},
			text:function(){return "× "+this.mod().format(2);}
		},
		{
		...
		},
		{
			...
			mod:function(){return AchievementE(518)?achievement(518).effect():N(1);},
			func:function(prev){return prev.mul(this.mod());},
			text:function(){return "× "+this.mod().format(2);}
		},
		{
...
breakdownStats.stardustMultiplier={
	...
	modifiers:[
		...
		},
		{
			...
			mod:function(){return AchievementE(412)?achievement(412).effect():N(1);},
			func:function(prev){return prev.mul(this.mod());},
			text:function(){return "× "+this.mod().format(2);}
		},
		{
...
breakdownStats.baseMasteryPowerExponent={
	...
	modifiers:[
		...
		},
		{
			...
			mod:function(){return AchievementE(529)?achievement(529).effect():N(1);},
			func:function(prev){return prev.mul(this.mod());},
...
breakdownStats.SAxisEffect={
	...
	modifiers:[
		...
		},
		{
			...
			mod:function(){return AchievementE(526)?achievement(526).effect():N(0);},
			func:function(prev){return prev.add(this.mod());},
			text:function(){return "+ "+this.mod().format(3);}
		},
		{
			label:achievement.label(505),
			mod:function(){return AchievementE(505)?achievement(505).effect().div(100).add(1):N(1);},
			func:function(prev){return prev.pow(this.mod());},
			text:function(){return "^ "+this.mod().format(4);}
		}
	]
...
breakdownStats.freeYAxis={
	...
	modifiers:[
		...
		},
		{
			...
			mod:function(){return AchievementE(206)?achievement(206).effect():N(0);},
			func:function(prev){return prev.add(this.mod());},
			text:function(){return "+ "+this.mod().format(2);}
		},
...
breakdownStats.tickspeed={
	...
	modifiers:[
		...
		},
		{
			...
			mod:function(){return [409,410,411].map(x => AchievementE(x)?achievement(x).effect().div(100).add(1):N(1)).reduce((x,y)=>x.mul(y));},
			func:function(prev){return prev.mul(this.mod());},
			text:function(){return "× "+this.mod().format(3);}
		},
		{
...
const stat = {}
for (let i of Object.keys(breakdownStats)) {
	statGenerations[i] = null
	stat[i] = null
}
function statGeneration(name) {
	if (typeof statGenerations[name] == "number") return statGenerations[name]
	let str = breakdownStats[name].modifiers.map(x => Object.values(x).join("")).join("")
	let highestDependency = -1
	while (str.indexOf("stat.") > -1) {
		let location = str.indexOf("stat.")+5
		let nameLength = 1
		let parent
		while (true) {
			let name = str.slice(location,location+nameLength)
		...
		highestDependency = Math.max(highestDependency, statGeneration(parent))
		str = str.slice(location+parent.length)
	}
	statGenerations[name] = highestDependency + 1
	return highestDependency + 1
}
var statOrder
function updateStats() {
	for (let id of statOrder) {
		let data = breakdownStats[id].modifiers
		let value = data[0].func()
		for (let i of data) value = i.func(value)
		stat[id] = value
	}
}
...
function statBreakdownCategories() {
	let categories = Array.from(new Set(Object.values(breakdownStats).map(x => x.category)));
	let out = {}
	for (let i of categories) {
		let contents = Array.from(new Set(Object.keys(breakdownStats).filter(x => breakdownStats[x].category==i)))
...
var activeBreakdownTabModifierLength = breakdownStats[activeBreakdownTab].modifiers.length
const maximumSSBModifierLength = Object.values(breakdownStats).map(x => x.modifiers.length).reduce((x,y)=>Math.max(x,y))
function toggleActiveBreakdownSection(x) {
	...
	d.innerHTML("SSBnav2",(breakdownCategories[x].contents.length==1)?"":(breakdownCategories[x].contents.map(x => "<button id=\"button_SSBnav2_"+x+"\" class=\"tabtier4\" style=\"filter:brightness("+(x==activeBreakdownTab?100:75)+"%)\" onClick=\"toggleActiveBreakdownTab('"+x+"')\">"+breakdownStats[x].label+"</button>").join("")))
...
function toggleActiveBreakdownTab(x){
	...
	activeBreakdownTabModifierLength = breakdownStats[x].modifiers.length
