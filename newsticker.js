...
const newsList = [
	...
	{get text(){return "In 2019, the United Nations estimated the average human lifespan across planet Earth was 72.6 years. What the United Nations did not take into consideration, though, is that exotic matter wormholes have made time tick "+(stat.tickspeed.gt(c.d1)?(stat.tickspeed.format(3)+"× faster"):(stat.tickspeed.recip().format(3)+"× slower"))+". Consequently, that figure is "+(stat.tickspeed.gt(c.d1)?"reduced":"increased")+" to approximately "+timeFormat(this.num.div(stat.tickspeed))+"."+((overclockActive&&stat.tickspeed.gt(c.d1))?(" A new phenomenon has also been discovered called 'overclocking', which further reduces this value to "+timeFormat(this.num.div(stat.tickspeed.mul(overclockSpeedupFactor)))+"."):"")},get weight(){return stat.tickspeed.log10().abs().min(c.d1).toNumber()},num:N(2291032826.87)},
