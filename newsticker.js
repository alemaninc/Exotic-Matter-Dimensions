const newsSupport = {
  ...
  nodeDocumentary:function(){
    ...
    if (parentType == null) {
      out += "But what really makes this node stand out, is that it is the very first node to be created of them all - it has neither a parent, nor a mother-in-law. What it does have is "+numword(children.length)+" child"+(children.length==1?"":"ren")+" - "+Array.joinWithAnd(Array.from(children).map(x=>x.tagName))+" - who are vitally important to the functioning of the game itself."
    } else {
      if (children.length>0) out += " It is happy to have a "+parentType+" parent - perhaps less so a "+list[Math.floor(list.length*Math.random())].tagName+" mother-in-law - and "+numword(children.length)+" child"+(children.length==1?"":"ren")+" named "+Array.joinWithAnd(Array.from(children).map(x=>x.tagName))+"."
...
const newsList = [
  ...
  {text:"The R axis is a lie.",get weight(){return axisUnlocked()==8?1:0}},
  ...
  {text:"Discovery №1: The planets move.",get weight(){return g.totalDiscoveries.gt(0)?0.5:0}},
  {text:"Discovery №2: Earth moves.",get weight(){return g.totalDiscoveries.gt(1)?0.5:0}},
  {text:"Discovery №3: C<sub>6</sub>H<sub>12</sub>O<sub>6</sub> + 6O<sub>2</sub> → 6H<sub>2</sub>O + 6CO<sub>2</sub> (+ energy)",get weight(){return g.totalDiscoveries.gt(2)?0.5:0}},
  {text:"Discovery №4: 6H<sub>2</sub>O + 6CO<sub>2</sub> → C<sub>6</sub>H<sub>12</sub>O<sub>6</sub> + 6O<sub>2</sub>",get weight(){return g.totalDiscoveries.gt(3)?0.5:0}},
  {text:"Discovery №5: Atom + atom = molecule.",get weight(){return g.totalDiscoveries.gt(4)?0.5:0}},
  {text:"Discovery №6: Mass ÷ mole = 6.02214076 × 10<sup>23</sup>",get weight(){return g.totalDiscoveries.gt(5)?0.5:0}},
  {text:"Discovery №7: The continents move.",get weight(){return g.totalDiscoveries.gt(6)?0.5:0}},
  {text:"Discovery №8: Playing this game is a hazard to the aquatic environment.",get weight(){return g.totalDiscoveries.gt(7)?0.5:0}},
  {text:"<s>Discovery №9: Evolution is sorcery made up by communists.</s>"+newsSupport.br(100)+"The 9th Discovery doesn't exist.",get weight(){return g.totalDiscoveries.gt(8)?0.5:0}},
  {text:"Discovery №10: Genes can jump.",get weight(){return g.totalDiscoveries.gt(9)?0.5:0}},
  {text:"Discovery №11: DNA is a 🧬 double helix.",get weight(){return g.totalDiscoveries.gt(10)?0.5:0}},
  {text:"Discovery №12: He who is a diabetic but forgets to take their insulin shall be condemned to a slow and certain death.",get weight(){return g.totalDiscoveries.gt(11)?0.5:0}},
  {text:"Discovery №13: X-rays are able to penetrate opaque black paper wrapped around a cathode ray tube, causing a nearby table to glow with fluorescence.",get weight(){return g.totalDiscoveries.gt(12)?0.5:0}},
  {text:"Discovery №14: If you drop a metric ton of bricks and a metric ton of feathers off a building, they will experience the same velocity. If you drop a metric ton of bricks and an imperial ton of bricks off a building, they too will experience the same velocity.",get weight(){return g.totalDiscoveries.gt(13)?0.5:0}},
  {text:"Discovery №15: c = (E ÷ m)<sup>0.5</sup>",get weight(){return g.totalDiscoveries.gt(14)?0.5:0}},
  {text:"Discovery №16: You should go make a Discovery in real life.",get weight(){return g.totalDiscoveries.gt(15)?0.5:0}},
  ...
  {get text(){return "Inflation rates by dimension: "+axisCodes.slice(0,axisUnlocked()).map(x => x+" Axis: "+axisCost(x,g[x+"Axis"].add(1)).div(axisCost(x)).sub(1).mul(100).format(2)+"%").join(newsSupport.br(100))},get weight(){return axisUnlocked()>3?1:0}},
  ...
  {text:"The Q Axis neither exists nor doesn't exist. In fact, absolutely nothing is known about the Q Axis, not even that absolutely nothing is known about the Q Axis.",get weight(){return axisUnlocked()<9?1:0}},
  ...
  {get text(){"You're so close to unlocking Dilation Upgrade "+(g.dilationUpgradesUnlocked+1)+"! You only need "+BEformat(dilationUpgrades[g.dilationUpgradesUnlocked+1].tickspeedNeeded)+"× tickspeed"},get weight(){return g.dilationUpgradesUnlocked==4?0:1}},
  ...
  {get text(){return "<div style=\"width:"+ranint(400,1200,true)+"px;height:1em;background-image:linear-gradient(90deg,rgba(0,0,0,0.5),rgba(255,0,0,0.5),rgba(255,255,0,0.5),rgba(0,255,0,0.5),rgba(0,255,255,0.5),rgba(0,0,255,0.5),rgba(255,0,255,0.5),rgba(255,0,0,0.5),rgba(0,0,0,0.5))\"></div>"}},
  {get text(){"Congratulations! You have reached the end of Exotic Matter Dimensions "+d.element("span_currentVersion").innerHTML+"! While we wait for alemaninc to produce another release, why don't you experience the fun all over again? It's really easy, just go to Options, press the big black button and input the password."}},
  ...
  {text:"\"But the R axis does exist! You just won't be able to experience it until around 𝕍6.9...\" - xhwzwka"+newsSupport.br(100)+"Little does xhwzwka know, that the R axis will enter the playing field as soon as 𝕍1.5.",get weight(){return ((axisUnlocked()==8)&&(g.RAxis==undefined))?1:0}},
  {get text(){return "Have you ever wondered what a news message looks like in reversed order? Here is a random news message, inverted: "+newsSupport.randomVisible().split("").reverse().join("")}},
  ...
  {text:"\""+["Oups!","Wensday","It's spelt Wensday, Right?","You spell it Wensday?","I guess I'll never knowm"].join(newsSupport.br(50))+"\" - xhwzwka after a long night of [REDACTED]"},
  {text:"<span onClick=\"g.newsTickerSpeed*=-1\">Click this to make the news ticker reverse direction</span>"},
  {visibleChars:Array.shuffle(countTo(7,true)).slice(0,2),get text(){return "\"JavaScript's not the best language, "+Array.random(["English","Polish","French","Python","Finnish"]).split("").map((x,i)=>this.visibleChars.includes(i)?"<span style=\"opacity:0.5\">#</span>":x).join("")+" is.\" - alemaninc"}}
]
var newsOrder = []
