...
const newsList = [
	...
	{get text(){return "So when are you going to complete Study "+(this.studyNum()===0?newsSupport.redactedFormat("[ERROR]"):roman(this.studyNum()))+"? It's at position "+Math.min(Math.ceil(researchRowsUnlocked()*(Math.random()+1)),researchRows+1)+"-8 on the research tree."},studyNum:function(){return visibleStudies().reduce((x,y)=>Math.max(x,y))+1},get weight(){return unlocked("Studies")}},
	...
	{text:"<span onMouseover=\"secretAchievementList[44].clicks++;if(secretAchievementList[44].clicks<100){currentNewsOffset+=ranint(200,400)*((currentNewsOffset*2>viewportWidth)?-1:1)}else{addSecretAchievement(44)}\">Try to click me!</span>"},
